/*
	...
	2. 문맥 자유 문법 종류
		...
		BNF 표기법 : 프로그래밍 언어의 구문을 정의,서술 시 가장 일반적인 표기법.
...
/*
	...
	-1980년대 : 추상 자료형과 객체지향-
		...
		0.Ada
		추상 자료형 패키지,병행 or 태스크 기능,예외처리 등 새로운 기능 도입

		단점: 언어의 크기 및 복잡성

		1.Modula-2
		범용 절차적 언어. 시스템 프로그래밍 목적 설계
		추상 자료형 모듈, 병행성 기능인 코루틴 기능 도입.
		작고 단순한 언어 지향

		2.Smalltalk
		일관적 방법으로 객체 지향 접근 위한 설계
		GUI 최초 지원.
		순수 객체지향 언어.

		3.C++
		C언어의 확장.
		C언어의 효율성을 유지하며 객체지향 프로그래밍 가능하도록 설계.
		포인터와 같은 중요 특징 포함됨.
	...
	-1990년대 : 인터넷 언어와 새로운 시도-
		...
		0.Python 
			...
			실사용률과 생산성 높음
			접근성과 응용력이 좋음 
			자연과학,공학,통계학,사회과학,빅데이터 등에 사용됨

		1. JAVA
		
			C언어와 유사.
			컴파일된 코드가 플랫폼 독립적.
			컴파일러가 Java 프로그램을 바이트 코드로 변환하고 바이트 코드를 실행하려면 자바 가상머신 필요.
			자바 가상머신을 설치할 수 있는 플랫폼에서 동일한 형태로 실행가능.

		3. 자바스크립트
			객체 기반 스크립트 언어
			웹 브라우저 등에 사용됨.
			다른 응용 프로그램의 내장 객체에 접근할 수 있는 기능 탑재
			서버 사이드 네트워크 프로그래밍 등 사용됨.
	...
	-2000년대 : 새로운 미래를 위하여-
		...
		0. C#
			C++,JAVA에서 많은 영향 받음.
			자바 가상머신처럼 닷넷 프레임워크 필요.
			C/C++ 보다 다소 느림

		1. Scala
			객체지향 및 함수형 요소가 결합된 다중패러다임 프로그래밍 언어
			자바 바이트코드 사용. -> 자바 가상머신 실행가능.
			Java 언어와 호환됨 -> JAVA API 사용가능

		2. Objective-C,Swift

			애플 iOS와 MacOs를 위한 언어
			LLVM로 빌드 및 같은 런타임 시스템 공유
			클로저,다중 리턴타입,네임스페이스,제네릭,타입유추 등 현대적 언어 기능 포함됨
			두 언어는 서로 상호작용 가능
/*
	4장. 프로그래밍 언어의 설계 원칙

		0. 효율성
			목적 코드의 효율성 : 번역기가 호율적인 실행코드 생성가능한지.
			번역의 효율성 : 번역기가 빠르게 번역될 수 있는지.
			구현 용이성 : 번역기가 효율적으로 만들어질 수 있는지.
			프로그래밍 효율성 : 빠르고 쉽게 프로그램을 만들 수 있는지.

		1. 일반성
			특별한 경우를 제외하고 관련있는 개념들이 일반적으로 결합하는 성질

		2. 직교성1
			문맥이 다르다고 다른 의미를 지니지 않아야 할 것.

		3. 획일성
			비슷한 것은 비슷하게,다른 것은 다르게 의미를 가져야 할 것.

		4. 간결성
			언어가 복잡하지 않아야 할 것

		5. 표현력
			복잡한 표현들을 쉽게 표현할 수 있는지 여부.

		6. 확장성
			언어에 새로운 기능을 추가할 수 있는지 여부

		7. 정확성
		   설계한 프로그램의 실행이 예측가능하도록 정확한 정의가 있는지 여부.

		8. 기계 독립성
			특정 기계에 의존하지 않고 독립적이어야 할 것.

		9. 제약성
			언어에 대해 최소한의 지식만 가지고도 프로그래밍이 가능해야할 것
